...
* `quaternion R`, `ℍ[R]` : the space of quaternions;
...
@[ext] structure quaternion (R : Type*) :=
(re : R) (im_i : R) (im_j : R) (im_k : R)
...
lemma coe_injective : function.injective (coe : R → ℍ[R]) :=
λ x y h, congr_arg re h

@[simp] lemma coe_inj {x y : R} : (x : ℍ[R]) = y ↔ x = y := coe_injective.eq_iff

@[simps short_name] instance : has_zero ℍ[R] := ⟨⟨0, 0, 0, 0⟩⟩

@[simp, elim_cast] lemma coe_zero : ((0 : R) : ℍ[R]) = 0 := rfl

instance : inhabited ℍ[R] := ⟨0⟩

@[simps short_name] instance : has_one ℍ[R] := ⟨⟨1, 0, 0, 0⟩⟩

@[simp, elim_cast] lemma coe_one : ((1 : R) : ℍ[R]) = 1 := rfl

@[simps short_name] instance : has_add ℍ[R] := ⟨λ a b, ⟨a.1 + b.1, a.2 + b.2, a.3 + b.3, a.4 + b.4⟩⟩

@[simps short_name] instance : has_neg ℍ[R] := ⟨λ a, ⟨-a.1, -a.2, -a.3, -a.4⟩⟩

@[simps short_name] instance : has_mul ℍ[R] := ⟨λ a b,
...
instance : ring ℍ[R] :=
by refine_struct { add := (+), zero := (0 : ℍ[R]), neg := has_neg.neg, mul := (*), one := 1 };
  intros; ext; simp only [] with quaternion_projs; ring_exp

instance : algebra R ℍ[R] :=
by refine @algebra.mk _ _ _ _ ⟨λ r (a : ℍ[R]), (⟨r * a.1, r * a.2, r * a.3, r * a.4⟩ : ℍ[R])⟩ coe
...
@[move_cast, simp] lemma coe_add : ((x + y : R) : ℍ[R]) = x + y :=
algebra.map_add ℍ[R] x y

@[move_cast] lemma coe_mul : ((x * y : R) : ℍ[R]) = x * y :=
algebra.map_mul ℍ[R] x y
...
lemma smul_coe : x • (y : ℍ[R]) = ↑(x * y) := by rw [coe_mul, coe_mul_eq_smul]

/-- Quaternion conjugate. -/
@[simps] def conj (a : ℍ[R]) : ℍ[R] := ⟨a.1, -a.2, -a.3, -a.4⟩
...
@[simp] lemma conj_coe : conj (x : ℍ[R]) = x :=
...
def conj_lin_equiv : ℍ[R] ≃ₗ[R] ℍ[R] :=
{ to_fun := conj,
  inv_fun := conj,
  left_inv := conj_conj,
  right_inv := conj_conj,
...
@[simp] lemma conj_zero : conj (0:ℍ[R]) = 0 := (conj_lin_equiv R).map_zero

@[simp] lemma conj_neg : (-a).conj = -a.conj := (conj_lin_equiv R).map_neg a

@[simp] lemma conj_sub : (a - b).conj = a.conj - b.conj := (conj_lin_equiv R).map_sub a b
...
def conj_ring_equiv : ℍ[R] ≃+* (ℍ[R]ᵒᵖ) :=
{ to_fun := op ∘ conj,
  inv_fun := conj ∘ unop,
  map_mul' := λ x y, by simp,
 .. (conj_lin_equiv R).to_add_equiv.trans (add_equiv.op_equiv ℍ[R]) }
...
{ to_fun := λ a, (a * a.conj).re,
  map_one' := by rw [conj_one, one_mul, has_one_re],
...
by simp only [norm_sq_def, pow_two, ← neg_mul_eq_mul_neg, sub_neg_eq_add] with quaternion_projs
